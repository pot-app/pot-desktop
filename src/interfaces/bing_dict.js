...
export async function translate(text, from, to, setText, id) {
    ...
    const res = await fetch(`https://www.bing.com/api/v6/dictionarywords/search?q=${text}&appid=371E7B2AF0F9B84EC491D731DF90A55719C7D209&mkt=zh-cn&pname=bingdict`);
    if (res.ok) {
        ...
        const formatGroups = meaningGroups.reduce((acc, cur) => {
        ...
        if (translateID.includes(id)) {
            setText(Object.keys(formatGroups).map(k => {
                const v = formatGroups[k];
                return v.flatMap(mean => {
                    ...
                    return richDefinitions.map(def => def.fragments.map(f => f.text).join('; ')).filter(v => v.trim().length > 0).map(v => partsOfSpeech + '. ' + v);
                }).join('\n');
